...
module.exports.run = async (bot, msg, args) => {
    ...
    else
    {
        ...
        switch(args[2]){ //il n'y a que 5 jours, on peut se permettre de faire un switch
            case 'lundi':
                bot.horaires[msg.channel.id].lundi = args[2];
                fs.writeFile("./horaires.json", JSON.stringify(bot.horaires, null, 4), err =>{ //on oublie pas de save ca !
                    if(err) throw err;
                    msg.channel.send(`nouvel horaire du **${args[2]}** : *${}*`)
                });
                break;
            ...
            default:
                msg.channel.send(`${args[2]} n'est pas un jour valide !`);
        }
    }
}
